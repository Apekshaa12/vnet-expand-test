...
function fetch-nsgnames
{
    ...
    $patterns = @(
        "$rgScope-$rgEnvironment-g-$rgDepartment-$rgRegionCode-*-nsg",
        "$rgScope-$rgEnvironment-c-$rgDepartment-$rgRegionCode-*-nsg"
    )
...
{ 
    ...
    $gitClone           = git clone "https://bogususer:$adoPat@dev.azure.com/nestle-it/STIG/_git/configs"

    $cd                 = cd "$sourceDirectory/configs/"
    ...
    foreach ($nsgName in $nsgNames)
    {
        ...
        $filePath   = "$sourceDirectory/configs/tf/az/$folderName/$nsgName.tf"
...
function update-nsg-rule
{
    ...
    while ($retryCount -le $maxRetries)
    {
	    ...
	    #git push origin $global:tempBranch
		git checkout master
		git pull origin master
    ...
    {
        Write-Host "Operation failed after $maxRetries retries. Pipeline failed to add the NSG inbound rule to the configs repo due to git merge issues"
 	$failedNSGCount   = 0
        $nsgUpdateFailed  = 0
        $successNSGCount  = 0
        $nsgUpdateSuccess = 0
